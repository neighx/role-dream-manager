import { DailyCheckin, Role, GeneratedTodayTask, Quadrant } from "@/types";
import { getEnergyConfig } from "./rules/energyRules";
import { findGapTemplates, selectTask } from "./rules/gapToTasks";
import { buildTodayReason } from "./rules/todayReasonBuilder";

// ════════════════════════════════════════════════════════════
// generateTodayPlan — AI差替え口
// MVP: ルールベース
// v2: anthropic.messages.create() に差替え
// ════════════════════════════════════════════════════════════
...
): Promise<GeneratedTodayTask[]> {
  return generateByRules(input);
}

// ── ルールベース生成 ─────────────────────────────────────────
function generateByRules(input: TodayPlanInput): GeneratedTodayTask[] {
  ...
  for (const role of selectedRoles) {
    ...
    for (const template of gapTemplates) {
      ...
      // longTermConnectionのプレースホルダーを置換
      ...
      results.push({
        id: `gen-${role.id}-${template.gapKeyword}-${Date.now()}`,
        role_id: role.id,
...
function classifyQuadrant(importance: number, urgency: number): Quadrant {
  if (importance >= 4 && urgency >= 4) return 1; // 重要・緊急
  if (importance >= 4 && urgency < 4) return 2;  // 重要・緊急でない
  if (importance < 4 && urgency >= 4) return 3;  // 緊急・重要でない
  return 4;                                        // どちらでもない
}
